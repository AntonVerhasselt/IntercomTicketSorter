...
const prepareMessagesForGPTPrompt = (conversationData, language) => {
  const systemMessageContent =
    categoryPrompts.generateCategorizeInstruction(language);
  const messages = [
    {
      role: "system",
  ...
  conversationData.conversation_parts.conversation_parts
    ...
    .forEach((part) => {
      const role =
        part.author && part.author.type === "admin" ? "assistant" : "user";
...
app.post("/webhook", (req, res) => {
  if (
    req.body.topic === "conversation.user.created" ||
    req.body.topic === "conversation.user.replied"
  ) {
    const conversationId = req.body.data.item.id;

    fetchConversationDetails(conversationId)
      .then(({ conversationData, language }) => {
        const messages = prepareMessagesForGPTPrompt(
          conversationData,
          language
        );
        sendPromptToGPT(messages).then((gptResponse) => {
          ...
          if (jsonResponse.category_id && jsonResponse.category_id !== "") {
            ...
            console.log(storeDataPayload);

            storeData(storeDataPayload, new Date().toISOString());
